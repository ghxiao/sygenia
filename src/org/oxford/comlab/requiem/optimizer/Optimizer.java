...
public class Optimizer {
	 ...
	 * @return
	 */
	public ArrayList<Clause> querySubsumptionCheck(ArrayList<Clause> clauses, ResolutionTracker tracker) {
		ArrayList<Clause> result = new ArrayList<Clause>();
		
		boolean c1IsSubsumed;
		
		/**
		 * Original Method contained a bug. The correct version additionally adds c1.toBeIgnored && !c2.toBeIgnored, 
		 * i.e. a clause that was previously identified to be ignored should not be used to ignore another clause.  
		 */
		//Identify equivalent clauses
		for (Clause c1 : clauses) {
			for (Clause c2 : clauses) {
				if(!c1.equals(c2) && !c1.toBeIgnored && !c2.toBeIgnored ) {
					if(c2.subsumes(c1,false) && c1.subsumes(c2,false) && tracker.isEligibleForSubsumptionBy( c1, c2)){
		...
		for (Clause c1 : clauses) 
			if(!c1.toBeIgnored){
				...
				for (Clause c2 : clauses) 
					if(!c2.toBeIgnored)
						if(!c1.equals(c2))
							if(c2.subsumes(c1,false) && tracker.isEligibleForSubsumptionBy(c2, c1)) {
								c1IsSubsumed = true;
								break;
							}
				if(!c1IsSubsumed)
