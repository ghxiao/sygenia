...
public class Resolution{
    ...
    public ArrayList<Clause> generateResolvents(Clause givenClause, Collection<Clause> workedOffClauses, ResolutionTracker tracker) {
        ...
        if(givenClause.m_selectedHead)
        	for (Clause workedOffClause : workedOffClauses) 
        		for (int mainPremiseIndex = 0; mainPremiseIndex < workedOffClause.getBody().length; mainPremiseIndex++)
        			...
        			if((workedOffClause.m_selectedBody[mainPremiseIndex]) && (workedOffClause.getBody()[mainPremiseIndex].getArity() == givenClause.getHead().getArity()) && (workedOffClause.getBody()[mainPremiseIndex].getName().equals(givenClause.getHead().getName()))){
        				Clause resolvent = resolve(workedOffClause, givenClause, mainPremiseIndex);
                        if (resolvent != null){
                        	tracker.trackDependencyForGeneratedClause( resolvent, givenClause, workedOffClause );
//                        	System.out.println( "workedOff: " + workedOffClause + " given:" + givenClause + " resolvent: " + resolvent );
        ...
        for (int mainPremiseIndex = 0; mainPremiseIndex < givenClause.getBody().length; mainPremiseIndex++)
        	if(givenClause.m_selectedBody[mainPremiseIndex])
        		for (Clause workedOffClause : workedOffClauses) 
        			...
        			if(workedOffClause.m_selectedHead && workedOffClause.getHead().getArity() == givenClause.getBody()[mainPremiseIndex].getArity() && workedOffClause.getHead().getName().equals(givenClause.getBody()[mainPremiseIndex].getName())){
        				Clause resolvent = resolve(givenClause, workedOffClause, mainPremiseIndex);
    					if (resolvent != null){
    						tracker.trackDependencyForGeneratedClause( resolvent, givenClause, workedOffClause );
//                        	System.out.println( " given:" + givenClause + " workedOff: " + workedOffClause + " resolvent: " + resolvent );
    						result.add(resolvent);
    					}
        			}
        return result;
    ...
    public Clause resolve(Clause mainPremise, Clause sidePremise, int mainPremiseAtomIndex) {
